...
#Drill through file structure and collect vaid files, compare with list of current files to halt duplicate uploads
try {
    [string[]]$Filearray1 = @()
    [string[]]$Filearray2 = @()
    ...
    $files = Get-ChildItem -Path $path -File -Recurse -Include $filetype -Exclude $Filearray2 -ErrorAction Stop
}
catch {
